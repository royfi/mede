...
def fruitMelomel():
    ...
    smaken = [smaak.title() for smaak in medeDict['smaken'].keys()]
    print(smaken)
    medeSmaak = input(
        'Van welke smaak uit de volgende lijst wilt u het recept zien? '
    ).strip().lower()
    
  

    while medeSmaak not in medeDict['smaken'].keys():
        medeSmaak = input(f'Kies een van de smaken uit de lijst {smaken}: ').strip().lower()
...
def recepten():
    ...
    #  of alleen belangrijke keys uit de dict pakken?
